...
// public:
//     rad_core_sse( const std::vector<plane> &planes, const std::vector<std::vector<float> > &ffs, const
//     std::vector<std::vector<int> > &ff_target )
...
{
public:
    rad_core_opencl(cl::Context ctx, cl::CommandQueue queue, const scene_static &scene_static,
                    const light_static &light_static)
        : ctx_(ctx)
        , queue_(queue)
        , num_planes_(scene_static.planes().size())
        , buf_dir_(true)
        , pints_(0)
        , pints_last_(0)
        , pints_last_time_(0)
    {
        ...
        {
            // std::for_each(light_static.f_fact().begin(), light_static.f_fact().end(), [&]( const std::vector<float> &
            // f) {
        ...
        queue_.enqueueWriteBuffer(col_diff_, false, 0, stage_col_diff.size() * sizeof(cl_float4),
                                  stage_col_diff.data());
...

        {
            ...
            queue_.enqueueWriteBuffer(f_rad_, false, 0, light_static.num_planes() * sizeof(cl_float4),
                                      stage_zero.data());
            queue_.enqueueWriteBuffer(f_rad2_, false, 0, light_static.num_planes() * sizeof(cl_float4),
                                      stage_zero.data());
        }
...
public:
        ...
        : pump_factor_(4)
    {
        ...
        //      CL_Mat4f proj = CL_Mat4f::ortho( -20.0 * pump_factor_, 20.0 * pump_factor_, -15.0 * pump_factor_, 15.0 *
        //      pump_factor_, 0, 200 );
...
//         render_unit runit2(is2, vec3f( 60.0, -20.0, 0.0 ));
        ...
        while (!do_quit)
        {
            ...
            //             for ( size_t i = 0; i < scene_static_.planes().size(); ++i ) {
            //                 plane &p = const_cast<plane &>(scene_static_.planes()[i]); // FIXME: HACK!!! is the
            //                 energy_rgp stored in the planes actually used? remove it!
...
//     }
    ...
    void init_opencl()
    {
        ...
        cl_context_properties props[] = {CL_GL_CONTEXT_KHR,
                                         ...
                                         CL_CONTEXT_PLATFORM,
                                         (cl_context_properties)cl_platform_(),
                                         0};
