...
void player::frame(double time, double dt)
{
    ...
    rot_y_ -=
        mouse_mapper_
            .delta_x(); // NOTE: rotations are CCW, so clockwise head rotation (to the right) means negative mouse delta
    rot_x_ -= mouse_mapper_.delta_y();

    //          CL_Mat4f rot = CL_Mat4f::rotate(CL_Angle(rot_x_, cl_degrees), CL_Angle(rot_y_, cl_degrees), CL_Angle(),
    //          cl_YXZ );
...
void input_mapper::add_mapping(int keycode, bool *indicator) { mappings_.push_back(mapping(keycode, indicator)); }
