...
class rad_core_threaded : public IRadCore
{
...
public:
    ...
    rad_core_threaded(const scene_static &scene_static, const light_static &light_static, const size_t num_threads)
        ...
        , emit_(light_static.num_planes())
        , rad_(light_static.num_planes())
        , rad2_(light_static.num_planes())
        ,
        ...
        , joiner_(&threads_)
    {
        ...
        if (!true)
        {
            threads_.push_back(std::thread([&]() { work(0, scene_static_.planes().size(), 0); }));
        ...
        else
        {
            ...
            for (size_t i = 0; i < num_threads; ++i)
            {
                ...
                threads_.push_back(std::thread([=]() { work(first, last, i); }));
            }
#else
    ...
    virtual ~rad_core_threaded()
    {
        ...
        std::cerr << "if the program hangs after this point, there is an error in the thread lifecycle management..."
                  << std::endl;
    ...
    virtual void copy(std::vector<vec3f> *out)
    {
        ...
        if (dt >= 1e6)
        {
            ...
            std::cout << "pint/s: " << dp / (dt / 1.0e6) << std::endl;
        }
    }
        ...
        for (int i = 0; i < steps; ++i)
        {
			...
			size_t target4 = targets[k+3];
                ...
                for (size_t k = sstart; k < send; ++k)
                {
                    const vec_t ff = vu::set1(ffs[k]);
                    const size_t target =
                        targets[k]; // target has to be pre-multiplied by 4 in light_static::do_preporcessing!
