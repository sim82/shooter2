...
plane::plane(plane::dir_type d, const vec3f &base_pos, const vec3i &pos, float scale, float energy)
    : dir_(d)
    , pos_(pos)
    , energy_(energy)
...
public:
    ...
    vec3i pos() const { return vec3i(x_, y_, z_); }
private:
    size_t x_, y_, z_;
...
#else
    for (auto dir :
         {plane::dir_zx_p, plane::dir_zx_n, plane::dir_yz_p, plane::dir_yz_n, plane::dir_xy_p, plane::dir_xy_n})
    {
...
#endif
void light_utils::render_light(std::vector<vec3f> *emitptr, const scene_static &scene, const vec3f &light_pos,
